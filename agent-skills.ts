...
async function loadSkills(): Promise<Skill[]> {
  try {
    ...
    return []; // skills フォルダが無くても動く
  }
}

// スキルの選び方：キーワード一致（速くて確実。ただし言い回しに弱い）
...
async function runAgent(userInput: string, skills: Skill[]) {
  // ★スキルを選び、選ばれたときだけ本文を差し込む（オンデマンド読み込み）
  ...
  for (let step = 0; step < 10; step++) { // 暴走防止に上限10回
    const res = await anthropic.messages.create({
      model: MODEL, max_tokens: 8192, system, messages, tools, // 全文書き込みは長くなるので上限を増やす
    });
    messages.push({ role: "assistant", content: res.content });

    // stop_reason だけで判断せず、道具の呼び出しが含まれているかで判断する。
    // （max_tokens で途中で切れた場合、stop_reason は "tool_use" にならないのに
    //   tool_use ブロックが残り、tool_result 未返却のまま履歴が壊れるため）
    ...
    for (const block of toolUses) {
      // 切れた呼び出しは引数が不完全なので、絶対に実行しない（中途半端な書き込み防止）
      if (truncated) {
        toolResults.push({
          type: "tool_result",
